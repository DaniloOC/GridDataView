  ...
  hdrs.forEach((h, i) => {
    if (i !== 0) {
      if (grouping.sum.includes(h.name)) {
        const somatorio = datas
          .map((d) => d[h.name])
          .reduce((pv, cv) => pv + cv, 0)
        listRow.push(
          createHeader(key + 'sum_' + i, { type: 'money', value: somatorio })
        )
      } else {
...
const createHeaders = (key, headers) => {
  ...
  const listHeaders = getHeaders(headers).map((header, _) => (
    <th key={uuid()} style={header.style}>
      {header.value}
    </th>
  ))
...
const createHeadersRows = (key, rows) => {
  return rows.map((row, index) => {
    if (Array.isArray(row)) {
      ...
      return (
        <tr key={uuid()}>{createHeader(key + 'headersrows_' + index, row)}</tr>
      )
    }
  })
}
  ...
  const config = {
    value: (
      <Form.Control
        ...
        name={'textfield_' + key}
        value={value}
        onChange={(event) => header.change(item, event.target.value)}
      />
    )
  }
